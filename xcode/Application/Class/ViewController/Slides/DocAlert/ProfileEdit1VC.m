...
#import "ProfileEdit1VC.h"
#import "UserVO.h"
#import "FaxManager.h"

@interface ProfileEdit1VC ()

@end

@implementation ProfileEdit1VC

@synthesize tf1, tf2, tf3;
@synthesize pagedot1, pagedot2, pagedot3, pagedot4, pagedot5;
@synthesize nextButton, backButton;

static NSString *tf1_default = @"FIRST NAME";
static NSString *tf2_default = @"MIDDLE NAME";
static NSString *tf3_default = @"LAST NAME";

- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    nibNameOrNil = @"ProfileEdit1VC";
...
- (void)viewDidLoad
{
    ...
    if ([DataModel shared].user != nil) {
        if ([DataModel shared].user.firstname != nil) {
            self.tf1.text = [DataModel shared].user.firstname;
        }
        if ([DataModel shared].user.middlename != nil) {
            self.tf2.text = [DataModel shared].user.middlename;
        }
        if ([DataModel shared].user.lastname != nil) {
            self.tf3.text = [DataModel shared].user.lastname;
        }
...
//    }
    ...
    if (isOk) {
        ...
        [DataModel shared].user.firstname = self.tf1.text;
        [DataModel shared].user.middlename = self.tf2.text;
        [DataModel shared].user.lastname = self.tf3.text;
        
        [_delegate gotoNextSlide];
    } else {
        [[[UIAlertView alloc] initWithTitle:@"INFO" message:@"Please enter first and last name." delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil] show];        
    }
}
