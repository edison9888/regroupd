...
#import "ProfileEdit3VC.h"
#import "UserVO.h"
#import "FaxManager.h"

@interface ProfileEdit3VC ()

@end

@implementation ProfileEdit3VC
...
static NSString *tf1_default = @"HOSPITAL ADDRESS";
static NSString *tf2_default = @"CITY";
static NSString *tf3_default = @"STATE/PROVINCE";
static NSString *tf4_default = @"ZIP CODE";


- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    nibNameOrNil = @"ProfileEdit3VC";
...
- (void)viewDidLoad
{
    ...
    if ([DataModel shared].user.company != nil) {
        self.label1.text = [DataModel shared].user.company;
    }
    if ([DataModel shared].user.address != nil) {
        self.tf1.text = [DataModel shared].user.address;
    }
    if ([DataModel shared].user.city != nil) {
        self.tf2.text = [DataModel shared].user.city;
    }
    if ([DataModel shared].user.state != nil) {
        self.tf3.text = [DataModel shared].user.state;
    }
    if ([DataModel shared].user.zip != nil) {
        self.tf4.text = [DataModel shared].user.zip;
    }
...
- (IBAction)goNext
{
    ...
    if (isOk) {
        [DataModel shared].user.address = self.tf1.text;
        [DataModel shared].user.city = self.tf2.text;
        [DataModel shared].user.state = self.tf3.text;
        [DataModel shared].user.zip = self.tf4.text;
